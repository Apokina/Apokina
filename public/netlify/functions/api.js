...
export default async (req) => {
  ...
  try {
    ...
    // --- Restaurar un grupo desde una copia de seguridad exportada ---
    // No sobrescribe nunca un grupo que ya exista en el servidor (esos datos
    // en vivo son la fuente de verdad); solo reconstruye grupos que ya no
    // existan, para recuperar el acceso tras perder el dispositivo o los datos.
    if (action === 'restore_group') {
      const code = (payload.groupCode || '').toUpperCase().trim();
      const data = payload.data;
      if (!code) return json(400, { error: 'Falta el código de grupo' });
      if (!data || !Array.isArray(data.members)) return json(400, { error: 'La copia de seguridad no es válida' });

      const existing = await store.get(code, { type: 'json' });
      if (existing) {
        return json(200, { group: existing, restored: false });
      }

      const rebuilt = {
        code,
        name: (data.name || 'Mi grupo').toString().slice(0, 60),
      ...
      await store.setJSON(code, rebuilt);
      return json(200, { group: rebuilt, restored: true });
    }
