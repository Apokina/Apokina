(() => {
  ...
  // ---------------------------------------------------------------------
  // Copia de seguridad: exportar e importar todos los grupos del dispositivo
  // ---------------------------------------------------------------------
  ...
  async function exportBackup() {
    ...
    try {
      ...
      for (const g of device.groups) {
        try {
          const group = await apiGetGroup(g.code);
          groupsData.push({ code: g.code, memberId: g.memberId, memberName: g.memberName, data: group });
        } catch (e) {
          // Si un grupo concreto ya no se puede leer, seguimos con el resto.
        }
      }
      const backup = {
        app: 'apokina-la-pasta',
        version: 1,
      ...
      a.href = url;
      a.download = `apokina-copia-seguridad-${fecha}.json`;
      ...
      state.busy = false;
      state.homeError = 'No se ha podido exportar la copia de seguridad: ' + err.message;
      render();
    }
  }
  ...
  async function handleImportBackupFile(file) {
    ...
    try {
      ...
      let backup;
      try { backup = JSON.parse(text); } catch (e) { throw new Error('El archivo no es una copia de seguridad válida'); }
      if (!backup || !Array.isArray(backup.groups)) throw new Error('El archivo no tiene el formato esperado');

      let recovered = 0; // grupos que ya existían: solo se ha recuperado el acceso
      let rebuilt = 0; // grupos que ya no existían y se han reconstruido

      for (const entry of backup.groups) {
        if (!entry || !entry.code || !entry.data) continue;
        let result;
        try {
          result = await apiPost('restore_group', { groupCode: entry.code, data: entry.data });
        } catch (e) {
          continue; // si uno falla, seguimos con los demás
        }
        const group = result.group;
        const me = group.members.find((m) => m.id === entry.memberId) || group.members.find((m) => m.name === entry.memberName);
        rememberGroup(group.code, me ? me.id : entry.memberId, me ? me.name : entry.memberName, group.name);
        if (result.restored) rebuilt++; else recovered++;
      }

      state.device = loadDevice();
      state.busy = false;
      const parts = [];
      if (recovered) parts.push(`${recovered} grupo${recovered === 1 ? '' : 's'} recuperado${recovered === 1 ? '' : 's'}`);
      if (rebuilt) parts.push(`${rebuilt} reconstruido${rebuilt === 1 ? '' : 's'} desde la copia`);
      state.homeInfo = parts.length ? `Listo: ${parts.join(', ')}.` : 'No se ha podido restaurar ningún grupo de ese archivo.';
      render();
    } catch (err) {
      state.busy = false;
      state.homeError = 'No se ha podido importar la copia de seguridad: ' + err.message;
      render();
    }
  }
  ...
  const state = {
    ...
    homeError: null, // aviso de error al exportar/importar la copia de seguridad
    homeInfo: null, // aviso informativo al exportar/importar la copia de seguridad
  };
  ...
  // Sonido corto y simple ("¡pim!") al añadir gastos o pagos. Antes era un
  // "chin chin" de caja registradora; ahora es solo un tono breve generado al
  // vuelo, sin depender de ningún archivo de audio.
  ...
  // Desbloquea el audio en el primer toque del usuario (algunos móviles lo
  // exigen antes de dejar sonar nada).
  ...
  // ---------------------------------------------------------------------
  // Botón "atrás" del móvil / navegador: que retroceda pantalla a pantalla
  // (en vez de salir de golpe de la aplicación) hasta llegar a "Tus grupos",
  // y solo entonces, si se vuelve a pulsar atrás, que se cierre la app.
  // ---------------------------------------------------------------------

  // Profundidad de cada pantalla dentro de la navegación: 0 = inicio,
  // 1 = dentro de un grupo (o crear/unirse), 2 = una subpantalla del grupo.
  const SCREEN_DEPTH = {
    home: 0,
    create: 1,
    join: 1,
  ...
  const BACK_SUBSCREENS = ['addExpense', 'addMember', 'settle', 'settings', 'movementDetail'];

  let navDepth = null; // profundidad ya reflejada en el historial del navegador
  let navPushed = 0; // cuántas entradas de historial hemos añadido nosotros
  let navPendingPops = 0; // "popstate" que esperamos por navegación propia (no del usuario)

  // Se llama en cada render(): mantiene el historial del navegador
  // sincronizado con la pantalla actual, sin tocar ninguna otra lógica de
  // la aplicación.
  function syncBackHistory() {
    const depth = SCREEN_DEPTH[state.screen];
    if (depth === undefined) return; // splash u otras pantallas transitorias

    if (navDepth === null) {
      // Primera pantalla "de verdad" de esta sesión: la dejamos como base.
      history.replaceState({}, '', location.pathname);
      navDepth = depth;
      return;
    }
    if (depth === navDepth) return;

    if (depth > navDepth) {
      // Hemos entrado más adentro: añadimos una entrada nueva al historial.
    ...
    // Hemos retrocedido. Si ha sido el propio botón atrás quien lo ha
    // provocado, el navegador ya se ha movido solo: no hacemos nada más.
    if (navPendingPops > 0) {
      navDepth = depth;
      return;
    }
    // Si ha sido la propia app (un botón "atrás"/"inicio", o tras guardar
    // algo), movemos el historial del navegador la misma cantidad, para que
    // el botón físico de atrás siga funcionando bien después.
    const steps = Math.min(navDepth - depth, navPushed);
    navDepth = depth;
    if (steps > 0) {
      navPushed -= steps;
      navPendingPops++;
  ...
  window.addEventListener('popstate', () => {
    if (navPendingPops > 0) { navPendingPops--; return; }
    // Botón atrás físico (o del navegador): subimos un nivel dentro de la app.
    if (navPushed > 0) navPushed--;
    if (BACK_SUBSCREENS.includes(state.screen)) {
      navDepth = 1;
      state.screen = 'group';
      state.error = null;
      draft = null;
      viewingItem = null;
      render();
    } else if (state.screen === 'group' || state.screen === 'create' || state.screen === 'join') {
      navDepth = 0;
      goHome();
    }
    // Si ya estábamos en "Tus grupos", no hacemos nada: dejamos que el
    // sistema cierre la aplicación, tal como se espera del botón atrás.
  });
  ...
  function renderHome() {
    ...
    const backupNotice = state.homeError
      ? `<div class="error-banner">${escapeHtml(state.homeError)}</div>`
      : (state.homeInfo ? `<div class="info-banner">${escapeHtml(state.homeInfo)}</div>` : '');
    // Input de archivo oculto, compartido por el botón "Importar copia" de
    // ambas variantes de esta pantalla (con y sin grupos todavía).
    const backupInput = `<input type="file" accept="application/json,.json" id="backupImportInput" data-role="backup-import-input" class="visually-hidden-file" />`;
    ...
    if (groups.length === 0) {
      app.innerHTML = `
        <div class="center-col">
          ...
          <button type="button" class="link-btn" style="margin-top:18px;" data-action="import-backup" ${state.busy ? 'disabled' : ''}>📥 Restaurar una copia de seguridad</button>
          ${backupInput}
        </div>`;
      return;
    }
    ...
    app.innerHTML = `
      ...
      <div class="section screen-pad-bottom">
        ...
        <div class="btn-row" style="margin-top:10px;">
          <button class="btn btn-outline btn-sm" style="flex:1;" data-action="export-backup" ${state.busy ? 'disabled' : ''}>${state.busy ? 'Exportando…' : '📦 Exportar copia'}</button>
          <button class="btn btn-outline btn-sm" style="flex:1;" data-action="import-backup" ${state.busy ? 'disabled' : ''}>📥 Importar copia</button>
        </div>
        ${backupInput}
      </div>`;
  }
  ...
  app.addEventListener('click', (e) => {
    ...
    if (action === 'import-backup') { document.getElementById('backupImportInput')?.click(); return; }
  ...
  app.addEventListener('change', (e) => {
    ...
    if (e.target.matches('[data-role="backup-import-input"]')) {
      const file = e.target.files[0];
